...
var _default = config => {
  ...
  newConfig.url = (0, _buildURL.default)((0, _buildFullPath.default)(newConfig.baseURL, newConfig.url), config.params, config.paramsSerializer); // HTTP basic authentication
...
var displayMap = exports.displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    ...
    L.marker([loc.coordinates[1], loc.coordinates[0]]).addTo(map).bindPopup("<h1>".concat(loc.description, "</h1>"), {
      autoClose: false
    }).openPopup();
  });
  var bounds = L.latLngBounds(points).pad(0.5);
  map.fitBounds(bounds);
  map.scrollWheelZoom.disable(); // Add zoom functionality using keyboard shortcuts
...
var userPasswordForm = document.querySelector('.form-user-passowrd');
